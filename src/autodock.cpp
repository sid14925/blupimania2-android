...
BOOL CAutoDock::Start(int part)
{
	...
	if ( m_phase != ADKP_WAIT )  return FALSE;  // occup� ?
	...
	if ( m_phase == ADKP_WAIT )  return FALSE;  // rien d�marr� ?
...
BOOL CAutoDock::EventProcess(const Event &event)
{
	...
	{
		StopBzzz();  // stoppe le son de l'�lectro-aimant
	}
	else	// porte qq chose ?
	{
		StartBzzz();  // d�marre le son de l'�lectro-aimant
		PosBzzz();  // d�place le son de l'�lectro-aimant
...
// Met � jour les positions des diff�rentes parties.
...
// Met � jour la position du nombre d'utilisations restantes.
...
// Cherche si le sol est � niveau sous le piston.
...
// Regarde s'il est possible d'atteindre une position donn�e.
...
// D�place le son de l'�lectro-aimant.
...
void CAutoDock::ReadSituation()
{
	...
	if ( m_undo->ReadTokenInt("phase", phase) )
	{
		...
		m_progress = 0.0f;
		m_speed = 1.0f/0.001f;  // instantan� (ou presque)
