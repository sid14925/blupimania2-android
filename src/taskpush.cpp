...
BOOL CTaskPush::EventProcess(const Event &event)
{
	...
	if ( m_phase == TPU_ROLL )  // roule une sph�re ?
	{
...
//?								pos.y -= 1.0f+pp*7.0f;
			...
			if ( m_bGoal )
			{
				if ( m_progress >= 0.5f && m_pGoal != 0 )
				{
					...
					m_pGoal = 0;
					m_terrain->SetLockZone(pos, LZ_BOX);  // zone occup�e !
				}
			}
		}
...
Error CTaskPush::Start(int part, int nbTiles)
{
	...
	if ( m_object->RetStrong() < 0.0f )  // blupi englu� ?
	{
		m_maxBox = 1;
		m_bRepeat = FALSE;
	}
	...
	// situ�e � c�t� du tunnel (perpendiculairement au tunnel) !
	...
	if ( (m_pBox[0]->RetType() == OBJECT_BOX7 ||    // sph�re ?
	...
	// V�rifie si les caisses � pousser sont compatibles avec le pousseur.
...
// (*)	< 0.5 pour que m_terrain->SetZoneLock() lib�re toujours
//		la case initiale o� est la mine !
...
Error CTaskPush::IsEnded()
{
	...
	if ( m_phase == TPU_PUSH )
	{
		...
		if ( m_bMine )  // arriv�e sur une mine ?
		{
			pyro = new CPyro(m_iMan);
			pyro->Create(PT_FRAGT, m_pMine);  // destruction mine

			i = m_totalBox-1;
			m_pBox[i]->DeletePart(1);  // supprime les fl�ches
		...
		else if ( m_bHole )  // arriv�e dans un trou ?
		{
			if ( m_bColomnFall )  // colonne ?
			{
				if ( m_bSpace )
...
// Bloque toute la zone des op�rations.
...
// Retourne la position d'arriv�e d'une caisse.
...
D3DVECTOR CTaskPush::RetAngleFreeFall(float angle)
{
	...
	// Si n�cessaire, calcule l'amplitude de la rotation
	// lat�rale de la caisse.
	if ( m_randFreeFall == NAN )
	{
		m_randFreeFall = 0.0f;
		pos = RetBoxGoalPos(m_totalBox-1);

		if ( m_part%2 == 1 )  // fl�che ouest/est ?
		{
			if ( rand()%2 == 0 )
			{
				pos.z += PUSH_DIST;
	...
	// sens dans lequel elle est pouss�e.
	if ( m_part == 1 || m_part == 5 )  // fl�che ouest ?
	{
		angle3D.z = -angle;
		angle3D.x =  angle*0.2f*m_randFreeFall;
	}

	if ( m_part == 2 || m_part == 6 )  // fl�che nord ?
	{
		angle3D.x = -angle;
		angle3D.z =  angle*0.2f*m_randFreeFall;
	}

	if ( m_part == 3 || m_part == 7 )  // fl�che est ?
	{
		angle3D.z = angle;
		angle3D.x = angle*0.2f*m_randFreeFall;
	}

	if ( m_part == 4 || m_part == 8 )  // fl�che sud ?
	{
		angle3D.x = angle;
		angle3D.z = angle*0.2f*m_randFreeFall;
	}
