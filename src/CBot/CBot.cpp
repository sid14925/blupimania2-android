...
BOOL CBotInstr::Execute(CBotStack* &pj)
{
	...
	/*debug break removed*/;			// ne doit jamais passer par cette routine
...
{
	/*debug break removed*/;			// papa sait pas faire, voir les filles
	return FALSE;
}
...
void CBotLeftExprVar::RestoreState(CBotStack* &pj, BOOL bMain)
{
	...
	if ( var1 == NULL ) /*debug break removed*/;
...
//	if ( pile1 == EOX ) return TRUE;
	...
	if ( pile1->GivState() == 0 )
	{
		...
		CBotInstr*	p  = GivNext3();							// les diff�rentes formules
		int			nb = 0;
...
CBotInstr* CBotExpression::Compile(CBotToken* &p, CBotCStack* pStack)
{
	...
	//		if (type2 == CBotTypClass) type2 = -1;		// pas de classe
				 ...
				 (type1.Eq(CBotTypClass)   && type2.Eq(CBotTypClass)   ) )
			{
				...
				if ( c1 != c2 ) type2.SetType(-1);	// pas la m�me classe
...
//	if ( pile == EOX ) return TRUE;
	...
	if ( pile->GivState() == 0 )
	{
		if ( !((CBotExprVar*)m_Instr)->ExecuteVar(var1, pile, NULL, TRUE) ) return FALSE;		// r�cup�re la variable selon champs et index
