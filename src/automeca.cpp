...
BOOL CAutoMeca::EventProcess(const Event &event)
{
	...
	if ( m_type == OBJECT_BARRIER58 )  // futura �tuve ?
	{
		pos.x = 0.0f;
	...
	if ( m_type == OBJECT_BARRIER66 )  // tr�sor ?
	{
	...
	if ( m_type == OBJECT_BARRIER74 )  // labo �tuve basse ?
	{
		if ( m_lastParticule+0.05f <= m_time )
		{
			m_lastParticule = m_time;
	...
	if ( m_type == OBJECT_BARRIER75 )  // labo �tuve haute ?
	{
		if ( m_lastParticule+0.05f <= m_time )
		{
			m_lastParticule = m_time;
