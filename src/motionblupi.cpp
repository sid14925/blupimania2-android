...
Error CMotionBlupi::SetAction(int action, float time)
{
	...
	if ( time == 0.2f )  // valeur par d�faut ?
	{
...
// Cr�e un robot quelconque pos� sur le sol.
...
BOOL CMotionBlupi::Create(D3DVECTOR pos, float angle, ObjectType type)
{
	...
	if ( m_additional[ADD_HAT] != 0 )  // avec chapeau ?
	{
		...
		if ( m_additional[ADD_HAT] == 4 )  // visi�re ?
		{
			m_object->SetPosition(16, D3DVECTOR(-0.3f, 3.2f, 0.0f));
			m_object->SetAngleZ(16, 0.0f*PI/180.0f);
		}
...
//?	if ( !m_engine->IsVisiblePoint(m_object->RetPosition(0)) )  return TRUE;
	...
	if ( m_closeTime <= 0.0f )
	{
		...
		{
			UpdateFaceMappingEye(MBLUPI_CLOSE);  // yeux ferm�s
			m_closeTime = 0.1f+Rand()*0.1f;  // choix dur�e
		}
		else
		{
			UpdateFaceMappingEye(MBLUPI_OPEN);  // yeux ouverts
			m_closeTime = 2.0f+Rand()*2.0f;  // choix dur�e
		}
	}
...
//?			linVib.z = cirVib.x*1.0f;
	...
	if ( action == MBLUPI_TRUCK )  // port� par dock ?
	{
		// Le corps se tortille.
		cirVib.x = sinf(m_aTime*10.0f)*0.25f;
		cirVib.z = cosf(m_aTime*10.0f)*0.25f;

		// Correction pour que la t�te reste immobile sur l'aimant.
...
//?		rot = RotatePoint(-(m_object->RetAngleY(0)-PI*1.5f), rot);
	...
	if ( action == MBLUPI_OUPS )  // perd l'�quilibre ?
	{
	...
	if ( action == MBLUPI_AMAZE  ||
		 action == MBLUPI_AMAZE2 )
	{
		UpdateFaceMappingMouth(MBLUPI_ANGRY);  // f�ch�
		if ( m_progress >= 1.0f )
		{
			UpdateFaceMappingMouth(MBLUPI_HAPPY);
			m_actionType = MBLUPI_WAIT;
	...
	// Bouge tous les membres d'apr�s la table.
	j = action;
	if ( j <  0      )  j = 0;
	...
	// Gestion de l'inclinaison d'apr�s certains membres.
	if ( action == MBLUPI_ERROR  ||
		 action == MBLUPI_ERROR2 )
	{
		 ...
		 action == MBLUPI_PFIOU2 )
	{
		s = m_object->RetAngleY(1);
		cirVib.y = -s;
		cirVib.z = 10.0f*PI/180.0f;  // pench� en arri�re
		linVib.y -= 0.2f;
		linVib.x -= 0.3f;
	}
...
// Gestion des particules lorsque Blupi est englu�.
...
// Met � jour le mapping de la texture des yeux.
...
// Met � jour le mapping de la texture de la bouche.
...
// D�termine le son � utiliser pour la marche (pour un pas).
...
void CMotionBlupi::BrainFrameDemo()
{
	...
	if ( pos.z > -160.0f )
	{
		...
		dh =  250.0f;  // total mont�e
	}
	else
	{
		ih = -150.0f;  // hauteur initiale
		dh =  110.0f;  // total mont�e
	}
	delay = dh/10.0f;
