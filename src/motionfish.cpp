...
void CMotionFish::DeleteObject(BOOL bAll)
{
}
...
BOOL CMotionFish::EventFrame(const Event &event)
{
	...
	if ( m_phase == MFP_JUMP )
	{
		if ( progress < 1.0f )
		{
			...
			angle = (80.0f*PI/180.0f)-(160.0f*PI/180.0f)*progress;
			...
			if ( progress > 0.10f && !m_bPlouf1 )
			{
				...
				m_sound->Play(SOUND_PLOUF, pos, 0.4f);
			}
			...
			if ( progress > 0.75f && !m_bPlouf2 )
			{
				...
				m_sound->Play(SOUND_PLOUF, pos, 1.0f);
			}
		}
		else
		{
...
BOOL CMotionFish::SearchPosition()
{
#if DEBUG_JUMP
