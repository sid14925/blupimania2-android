...
// Cr�e le fichier blupimania.ini la premi�re fois.
...
// Retourne la phase courante (utilis� par la couche tactile du port).
...
void CRobotMain::ChangePhase(Phase phase, Phase fadeIn)
{
	...
	if ( phase == PHASE_LOST )
	{
		...
		DeselectAll();  // d�s�lectionne la voiture
//?		m_camera->StartVisit(m_engine->RetLookatPt(), 200.0f);
		...
		return;  // conserve la sc�ne en cours !
	}
...
//?		m_engine->SetOverColor();
	...
	if ( m_phase == PHASE_SIMUL )
	{
		...
		if ( bCreate )
		{
			CreateScene(FALSE);  // sc�ne interractive
		}
...
// Lit l'�tat des touches super-shift.
...
// Indique quelle est la touche super-shift 1..9 press�e.
...
void CRobotMain::ExecuteCmd(char *cmd)
{
	...
	if ( m_phase == PHASE_SIMUL )
	{
		if ( strcmp(cmd, "bypass") == 0 )
		{
			...
			m_bCheatUsed = TRUE;  // gagn� en trichant
			return;
		}
...
//?	if ( strcmp(cmd, "showlockzone") == 0 )
	...
	if ( strcmp(cmd, "crypt") == 0 )
	{
		Crypt("scene", TRUE);
		Crypt("defi", TRUE);
		m_displayText->DisplayText("Cryptage termin�");
	}
	if ( strcmp(cmd, "decrypt") == 0 )
	{
		Crypt("scene", FALSE);
		Crypt("defi", FALSE);
		m_displayText->DisplayText("D�cryptage termin�");
	}
...
// Crypte ou d�crypte tous les fichiers.
...
// Stoppe l'action en cours d�s que possible.
...
// D�but de l'affichage des indices.
...
// D�but de l'affichage des instructions.
...
// D�placement pendant une s�lection.
...
// Op�ration sur le groupe d'objets s�lectionn�s.
...
void CRobotMain::GroupOper(FPOINT mouse)
{
	...
	if ( RetSelect() == 0 )  return;  // rien de s�lectionn�

	if ( pTarget == 0 )
	{
		ppos = pos;
		ppos.y = m_edit->RetGridHeight();
		m_particule->CreateGoto(ppos);  // montre la position d'arriv�e
	}
	...
	for ( i=0 ; i<1000000 ; i++ )
	{
		...
		if ( pObj->RetLock() )  continue;  // par ex. soulev� par dock
...
// D�tecte une position au sol ou un objet.
	...
	if ( m_terrain->GroundDetect(mouse, pos) )
	{
		...
		if ( pObj != 0 )
		{
			...
			if ( type == OBJECT_LIFT      ||
				 ...
				 type == OBJECT_BARRIER65 )   // barri�re ?
			{
				pObj = 0;
			}
		}
...
BOOL CRobotMain::DetectObject(FPOINT pos, CObject* &pObj, int &part)
{
	...
	// D�tecte tous les objets.
	if ( !DetectObject(pos, FALSE, pObj, part) )  return FALSE;

	pObjBase = pObj;
	partBase = part;
	// D�tecte seulement les boutons-actions.
	if ( DetectObject(pos, TRUE, pObj, part) )  return TRUE;

	// Si pas trouv� un bouton-action, rend l'objet de base.
	pObj = pObjBase;
	part = partBase;
	return TRUE;
}
...
BOOL CRobotMain::DetectObject(FPOINT pos, BOOL bAction, CObject* &pObj, int &part)
{
	...
	for ( i=0 ; i<1000000 ; i++ )
	{
		...
		// Il faut d�tecter une caisse "lock" (pouss�e), sinon
		// on va tenter un "goto" � cet endroit !
...
// Efface certaines mises en �vidence lorsque la souris a boug�.
	...
	if ( m_phase == PHASE_SIMUL )
	{
		...
		if ( m_winDelay > 0.0f )
		{
			...
			if ( m_winDelay <= 0.0f )
			{
				...
				m_event->MakeEvent(newEvent, EVENT_WIN);
				m_event->AddEvent(newEvent);
				m_bCheatUsed = FALSE;  // gagn� sans tricher
			}
		}
...
void CRobotMain::CreateScene(BOOL bFixScene)
{
	...
	InitEye();
	StartLift();  // d�marre les ascenseurs
}
...
// Limite la quantit� d'un objet quelconque en d�truisant qq objets.
...
void CRobotMain::CreateLimit(ObjectType type, int max)
{
	...
	for ( i=0 ; i<index-max ; i++ )
	{
		...
		list[j]->DeleteObject(TRUE);  // d�truit rapidement
		delete list[j];
		list[j] = 0;
	}
}
...
// Retourne un param�tre additionnel pour blupi.
...
// V�rifie si le puzzle est plausible.
...
BOOL CRobotMain::WriteScene()
{
	...
	// Ecrit les tokens interm�diaires.
	for ( i=0 ; i<100 ; i++ )
	{
...
// D�marre tous les ascenseurs. Le premier ascenseur d�marr� va
// s'occuper des mouvements de la cam�ra pour le film initial.
...
Error CRobotMain::CheckEndMission(BOOL bFrame)
{
	...
	nbGoal += nbWin;  // car les ballons ont �t� d�truits !
	...
	if ( nbWin >= m_totalBlupi )  // tous les blupi ont trouv� les ballons ?
	{
		m_winDelay  = 0.1f;  // gagn� dans 0.1 seconde
		m_lostDelay = 0.0f;
		m_displayText->SetEnable(FALSE);
...
// Indique s'il faut utiliser toutes les poign�es-actions.
...
// Retourne le r�sum� pour une langue donn�e.
...
// Retourne le r�sum� dans une langue donn�e.
...
// Retourne le r�sum� dans la langue courante, ou dans une autre
// langue existante.
...
// Sp�cifie le nombre de manipulations.
