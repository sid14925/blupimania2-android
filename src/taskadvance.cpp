...
// Le mode bNoError=TRUE est utilis� lorsque la t�che est cr��e
// suite � un Undo. Dans ce cas, la position d'arriv�e m_goalPos
// peut �tre occup�e selon IsPosFree par le robot lui-m�me. Et
// il ne faut pas la consid�rer comme occup�e !
...
//		m_object->SetLock(TRUE);
	...
	// La position de start est d�j� lock.
...
Error CTaskAdvance::IsEnded()
{
	...
	{
		// On laisse lock la position d'arriv�e.
		m_terrain->SetLockZone(m_startPos, LZ_FREE);
		...
		if ( m_bGoal )
		{
			...
			m_object->SetLock(TRUE);  // l'instruction "islock" le d�tecte !
