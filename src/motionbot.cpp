...
void CMotionBot::CreatePhysics()
{
	...
	if ( type == OBJECT_BOT4 )
	{
		short member[] =
		{
			  ...
			  0,   0, -40,  // jambe gauche
			  0,   0,  20,  // pied gauche
			-15,  20,   0,  // bras droite (bras �cart�s)
	...
	if ( type == OBJECT_CRAZY )
	{
		short member[] =
		{
		   ...
		   -140,   0,   0,  // bras gauche
			...
			-90, -30,  30,  // bras droite
			  0,   0,   0,  // main droite sup
			  0,   0,   0,  // main droite inf
			  0,   0,   0,  // bras gauche (tendu � l'horizontale)
			  0,   0,  30,  // bras gauche
			  0,   0,   0,  // main gauche sup
			  0,   0,   0,  // main gauche inf
							// MBOT_WALK1:
		...
		short member_crazy2[] =		// bras crois�s
		{
							// MBOT_WAIT:
			  0,   0,   0,  // corps
			  0,   0, -10,  // coup
			  ...
			  0,   0, -40,  // jambe gauche
			  0,   0,  20,  // pied gauche
			-30, -55,   0,  // bras droite (bras crois�s)
...
//?	if ( !m_engine->IsVisiblePoint(m_object->RetPosition(0)) )  return TRUE;
	...
	if ( type == OBJECT_BOT1 )
	{
		if ( action == MBOT_WAIT )
		{
			...
			effect[3*3+xx] = sinf(m_aTime*9.0f)*0.04f;  // t�te
...
//?			time *= 4.0f;
		...
		if ( c != 0.0f )  // tourne ?
		{
			...
			if ( c > 0.5f )
			{
					 ...
					 m_option == 3 )   // ss ?
				{
					...
					effect[3* 2+yy] =  sinf(m_aTime*10.0f)*0.20f;  // t�te
				...
				else
				{
					...
					effect[3* 2+yy] =  sinf(m_aTime*5.0f)*0.2f;  // t�te
		...
		if ( action == MBOT_YOUPIE )  // trouv� ballon ?
		{
...
//?		m_object->SetAngleZ(i, m_armAngles[i*3+zz+j*3*max]*PI/180.0f+effect[i*3+zz]);
	...
	if ( type == OBJECT_BOT2 )
	{
		if ( m_actionType == MBOT_WAIT )
		{
			...
			// des pieds pour que les pieds restent plaqu�s au sol.
...
// Cerveau du robot, pour �viter l'usage d'un script cbot.
...
// G�n�re les particules du robot fou.
...
// Fait feu (moyeu) avec la trompe du m�chant #1.
...
// Fait feu (tr�s efficace) avec la trompe du m�chant #1.
...
// (*)	CTaskAdvance::Start peut g�n�rer une erreur si le robot
//		n'est pas � la position de d�part. Si le robot est proche
//		de la position d'arriv�e, il consid�re cette position
//		comme occup�e (par lui-m�me) !
