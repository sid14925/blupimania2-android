...
#define KEY_TOTAL		8		// nb total de touches red�finissables
#define WELCOME_LENGTH	6.0f




// Construit la cl� d'un puzzle "officiel" pour CGamerFile.
...
// Indique si les boutons d�fi/atelier sont accessibles.
...
void CMainDialog::ChangePhase(Phase phase, Phase fadeIn)
{
	...
	if ( m_phase == PHASE_DEFI )
	{
		...
		pa->SetState(STATE_EXTEND);  // x/v tout � droite
		...
		pa->SetTabs(3, 145.0f/640.0f, 1, FONT_COLOBOT);  // resum�
	...
	if ( m_phase == PHASE_USER )
	{
		...
		pa->SetTabs(0, 110.0f/640.0f, 1, FONT_COLOBOT);  // filename
		pa->SetTabs(1,  70.0f/640.0f, 1, FONT_COLOBOT);  // univers
		pa->SetTabs(2,  45.0f/640.0f, 1, FONT_COLOBOT);  // test�
		pa->SetTabs(3, 190.0f/640.0f, 1, FONT_COLOBOT);  // resum�
		pa->SetSelectCap(TRUE);
		pa->SetFontSize(9.0f);
		UpdateSceneList(m_sel);
		pa->SetTabOrder(7);
...
//?			m_engine->SetOverColor(RetColor(intensity), mode);
		...
		// Port: l'�cran d'adieu (PHASE_GENERIC1) se ferme tout seul apr�s
...
// Fait �voluer qq jolies particules.
...
// Indique le niveau de difficult�.
...
// Initialise le personnage de base lors de la cr�ation d'un joueur.
...
void CMainDialog::FlushPerso()
{
	...
	m_perso.total = 1;
	m_perso.bonus = 1;
	m_perso.level = 3;  // niveau interm�diaire (costaud)
}
...
// Lit les caract�ristiques d'un fichier.
...
// Indique si un d�fi est accessible.
...
// Met � jour la liste des missions.
...
void CMainDialog::UpdateSceneList(int &sel)
{
	...
	pa->SetName(-1, name);  // texte de la l�gende
	...
	pa->SetSelect(sel);  // s�lectionne comme pr�c�demment
	pa->Sort();
	pa->ShowSelect();  // montre
}

// Met � jour les boutons selon la liste.
...
// Met � jour les boutons selon l'ascenseur.
...
// Met � jour les boutons des puzzles.
				...
				for ( level=0 ; level<4 ; level++ )
				{
					...
					if ( m_sel == aChap*16+group*4+level )
					{
						...
						if ( bInit )
						{
							if ( m_movePhase != 20 )
							{
								if ( group == 0 && level == 0 )  // nouvel �tage ?
								{
									m_moveCenter.x =  0.5f;
//?									m_moveCenter.y =  1.5f;
									m_moveCenter.y = -0.5f;
...
// Voulez-vous d�truire le puzzle ?
...
// Voulez-vous cr�er un nouveau puzzle ?
...
// Met � jour le dialogue pour cr�er un nouveau puzzle.
...
// Informations compl�mentaires sur le puzzle.
...
// Met � jour le type d'exportation.
...
void CMainDialog::StartImportPuzzle()
{
	...
	pa->SetTabs(1,  70.0f/640.0f, 1, FONT_COLOBOT);  // univers
	pa->SetTabs(2,  70.0f/640.0f, 1, FONT_COLOBOT);  // auteur
	pa->SetTabs(3, 180.0f/640.0f, 1, FONT_COLOBOT);  // resum�
	pa->SetSelectCap(TRUE);
	pa->SetFontSize(9.0f);
...
// Met � jour la liste des puzzles � importer.
...
// Indique si l'on �dite un puzzle.
...
// Signal la mission comme r�ussie.
...
// Indique si la mission est r�ussie.
...
// Cherche la mission suivante par d�faut � effectuer.
...
void CMainDialog::NextMission()
{
	...
	// Cherche dans tout l'�tage de 16 une pas faite.
	first = (m_sel/16)*16;
	...
	// Cherche dans l'�tage suivant une pas faite.
...
// Indique s'il faut �crire le fichier.
...
// Cr�e un nouveau puzzle sur la base d'un fichier mod�le.
...
// Supprime le puzzle s�lectionn� dans la liste.
...
// Supprime un fichier des d�fis pour tous les joueurs.
...
// Renomme le puzzle s�lectionn� dans la liste.
...
// Exporte le puzzle s�lectionn� dans la liste.
...
BOOL CMainDialog::ExportPuzzle(int i)
{
	...
	if ( m_exportType == 0 )  // exporte dans les d�fis ?
	{
...
// Ajoute dans un puzzle la commande indiquant s'il a �t� solutionn�.
