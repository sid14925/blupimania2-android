...
BOOL CTaskGoto::EventProcess(const Event &event)
{
	...
	if ( m_phase == TGP_GOTO && m_pathIndex >= 0 )  // goto dot list ?
	{
		...
		goal = m_pathPos[m_pathIndex];  // position � atteindre
		dist = Length2d(pos, goal);  // longueur � avancer
		...
		if ( Abs(dir) < 50.0f*PI/180.0f )  // presque fini de tourner ?
		{
			...
			if ( m_object->RetStrong() < 0.0f )  speed *= 0.7f;
			factor = 1.0f;
			if ( m_totalAdvance < m_linStopLength )  // premi�re droite ?
			{
				factor = 1.0f-(m_linStopLength-m_totalAdvance)/m_linStopLength;
				factor = 0.2f+factor*0.8f;
			}
			if ( m_pathIndex == 0 &&  // derni�re droite ?
	...
	if ( m_phase == TGP_RUSH )  // �lan initial ?
	{
		m_delay -= event.rTime;
	}
	...
	if ( m_phase == TGP_GOTO )  // goto dot list ?
	{
		...
		if ( m_pathIndex >= 0 )
		{
			if ( m_pathIndex == 0 )
			{
				...
				{
					limit = 0.2f;  // pr�cis si dernier point
				}
				else
				{
					limit = m_finalMargin;
			...
			{
				limit = 1.0f;  // moins pr�cis si pas dernier point
			}
			goal = m_pathPos[m_pathIndex];  // position � atteindre
			dist = Length2d(pos, goal);  // longueur � avancer
		}
		...
		if ( dist < limit )
		{
			...
			m_pathIndex --;  // index point suivant � atteindre

			if ( m_pathIndex >= 0 &&  // pas encore arriv� ?
				 !m_bStopPending )
			{
...
// Sp�cifie la progression pour une action pour Blupi.
...
// Sp�cifie la vitesse lin�aire pour une action pour Blupi.
...
// Sp�cifie la vitesse circulaire pour une action pour Blupi.
...
// Teste si un d�placement dans la case "goal" suivante est possible..
// Retourne TRUE si on ne peut pas atteindre la position.
// Retourne FALSE et bloque la position si le d�placement est possible.
...
// Dans une premi�re passe, essaye en �vitant les max1x ferm�s.
// Dans une deuxi�me passe, essaye m�me les max1x ferm�s.

BOOL CTaskGoto::PathFinder(D3DVECTOR start, D3DVECTOR goal)
{
	m_pass = 0;  // premi�re passe
	m_secondNeed = 0;
	if ( PathFinderPass(start, goal) )  return TRUE;

	if ( m_secondNeed == 0 )  return FALSE;
	m_pass = 1;  // deuxi�me passe
	return PathFinderPass(start, goal);
}

// Cherche le chemin le plus court, selon une id�e g�niale de Denis.
...
// Toutes ces cases visit�es sont ajout�es dans une liste (m_listTable),
// avec l'index de la case p�re.
//		S 1 2 X 6 7 8 G
//		1 1 2 X 5 X . .
//		2 2 2 3 4 X 8 8
//		3 X X 3 4 X 7 7
//		4 5 X 4 4 5 6 7
//		5 5 6 5 5 5 6 7
// D�s qu'on atteint "G", on remonte "8", "7" ... "1" en arri�re
// dans la liste (m_listTable) grace � l'index de la case p�re,
...
//		. S b
//		. . .
// On accepte d'aller de "S" � "1" si "a" ET "b" sont des obstacles
// arrondis (blupi, mine, sph�re, etc.).
...
BOOL CTaskGoto::PathFinderPass(D3DVECTOR start, D3DVECTOR goal)
{
	...
	ListCreate();  // cr�e la liste et le bitmap
	m_pathIndex = -1;  // corrige un bug cherch� longtemps avec Denis !!!
	...
	// Met tous les points "n+1" autour des pr�c�dents "n".
	...
	// R�cup�re le chemin le plus court depuis la fin.
	m_pathIndex = 0;
	i = m_listTotal-1;  // index derni�re case (c'est toujours le goal)
	while ( i != 0 )
	{
		x = m_listTable[i].x;
		y = m_listTable[i].y;
		...
		i = m_listTable[i].father;  // index case "n-1" p�re
	}
	m_pathIndex --;

	// Lock le premier segment qui sera avanc�.
	if ( m_pathIndex >= 0 )
	{
...
// D�termine si une position du terrain est libre.
...
BOOL CTaskGoto::IsTerrainFree(int ix, int iy, int x, int y)
{
	...
	if ( x == m_goalx && y == m_goaly )  // est-on sur l'arriv�e ?
	{
		if ( lz == LZ_MAX1X )
		{
			lz = LZ_FREE;  // si arriv�e = trappe -> comme si libre
		}
		if ( m_finalMargin != 0.0f )
		{
			lz = LZ_FREE;  // si arriv�e = potion -> comme si libre
		}
	}
	...
	if ( m_pass == 0 )  // premi�re passe ?
	{
		if ( lz == LZ_MAX1X )
		{
			m_secondNeed ++;  // une deuxi�me passe serait utile
			return FALSE;
		}
	}
	else	// deuxi�me passe ?
...
// D�termine si une position du terrain est libre ou occup�e
// par un objet rond.
...
// Cr�e une liste et un bitmap vide.

void CTaskGoto::ListCreate()
{
	ListFlush();  // lib�re si n�cessaire
...
// Ajoute un �l�ment � la fin de la liste, seulement s'il n'existe
// encore aucun �l�ment � cette coordonn�e.
// Retourne TRUE si on a atteint le goal et qu'il faut arr�ter.
