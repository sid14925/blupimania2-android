...
BOOL CAutoCatapult::Start(int part)
{
	...
	if ( m_phase != ACAP_WAIT )  return FALSE;  // occup� ?
	...
	if ( m_phase == ACAP_WAIT )  return FALSE;  // rien d�marr� ?
...
BOOL CAutoCatapult::EventProcess(const Event &event)
{
	...
	if ( m_phase == ACAP_RETURN )  // marteau au d�part ?
	{
...
// Bloque/d�bloque toute la zone de catapultage.
...
#else
// Finalement, on ne s'occupe que des positions de d�part et
// d'arriv�e, pour permettre de survoller une trappe sans qu'elle
// ne s'ouvre !
	D3DVECTOR	pos;
...
// Une valeur positive correspond � la direction de lancement.
