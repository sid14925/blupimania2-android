...
BOOL CTaskRoll::EventProcess(const Event &event)
{
	...
	if ( m_phase == TRO_ROLL )  // roule une sph�re ?
	{
		if ( m_bImpossible )
		{
			pos = m_startPos;
		...
		else
		{
			...
			if ( m_bHole )
			{
				...
				else if ( !m_bSpace )  // sph�re tombe dans trou ?
				{
					if ( progress > 0.8f )
					{
			...
			m_object->SetPosition(0, pos);  // avance la sph�re
...
Error CTaskRoll::Start(D3DVECTOR dir)
{
	...
	m_boxAngle = m_object->RetAngle(9);  // angle initial de la sph�re

	m_bMash = FALSE;
	m_bExplo = FALSE;
	m_pMash[0] = SearchObject(m_goalPos, 4.0f);

	// Si on n'a pas trouv� un objet immobile, cherche si la destination
	// est �ventuellement occup�e par un blupi mobile. En fait, par un
...
Error CTaskRoll::IsEnded()
{
	...
	if ( m_phase == TRO_ROLL )
	{
		...
		else if ( m_bHole )
		{
			...
			{
				m_object->DeletePart(1);  // supprime les fl�ches
			...
			else
			{
				...
				m_object->DeletePart(4);
				pyro = new CPyro(m_iMan);
				pyro->Create(PT_FRAGT, m_object);  // destruction sph�re
				return ERR_STOP;
			}
		}
		else
...
// Cherche les objets proche � casser aussi. Pour les vitres,
// on consid�re toute la ligne comme un seul objet.
...
// Fait trembler un objet rencontr� par la sph�re.
...
// (*)	Attention: Dans ce cas, la vitre est tourn�e de 90 degr�s.
//		Il faut donc bien utiliser l'axe "x" !

// Fait exploser un objet rencontr� par la sph�re.
...
void CTaskRoll::ExploObject()
{
	...
	for ( i=0 ; i<m_totalMash ; i++ )
	{
		...
		if ( i == 0 )  // l� o� est la boule ?
		{
			m_terrain->SetLockZone(pos, LZ_BOXo);  // zone occup�e !
		}
	}
